...
namespace BusTraveller.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<User>>> GetAllTravellers()
        {
            
                // Retrieve all users from the repository
                var users = await _userRepo.GetAll();

                // Filter users to get only travellers (users with UserRoleId == 2)
                var travellerUsers = users.Where(u => u.UserRoleId == 2);

                // Map traveller users to UserDto objects
                var travellerDtos = _mapper.Map<IEnumerable<User>, IEnumerable<User>>(travellerUsers);
